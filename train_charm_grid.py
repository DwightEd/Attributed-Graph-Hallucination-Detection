...
# ---------------------------------------------------
# Attention-based Charm Message Passing
# ---------------------------------------------------
class CharmMP(MessagePassing):
    def __init__(self, node_dim, edge_dim, hidden_dim, residual=True):
        super().__init__(aggr="add")
        ...
        # Attention weights for message passing
        # self.attention_mlp = nn.Linear(node_dim + edge_dim + 2, 1)
...
class CHARM(nn.Module):
    def __init__(self, base_dim, edge_dim, hp):
        ...
        # 为每个节点计算注意力权重的MLP
        # self.attention_mlp = nn.Linear(hp["hidden_dim"], 1)
...
def train_model(train_loader, val_loader, base_dim, edge_dim, hp):
    ...
    print(f"[Info] raw_pos_weight={pos_weight:.3f}")
    pos_weight = min(pos_weight_val, 8).to(DEVICE)
    print(f"[Info] raw_pos_weight={pos_weight:.3f}")
...
def compute_hallucination_ratio(dataset):
    ...
    for data in dataset:
        y = data.y
        if y.dim() == 0:  # Scalar case
            y_list.append(y.item())
        else:  # Token-level y; treat as hallucination if sum > 0
...
if __name__ == "__main__":
    ...
    TRAIN_DIR = "attributed_graphs_fixed/RAGtruth/llama2-7b-chat-hf/train_Data2txt"
    TEST_DIR  = "attributed_graphs_fixed/RAGtruth/llama2-7b-chat-hf/test_Data2txt"
    # VAL_DIR = "attributed_graphs_fixed/TruthfulQA/Mistral-7B-Instruct-v0.3/val_Multi_choices_QA"
    ACT_DIM=4096
    train_graphs = load_graph_dir(TRAIN_DIR)
    test_graphs  = load_graph_dir(TEST_DIR)
    # val_graphs = load_graph_dir(VAL_DIR)

    train_list = [graph_to_data(g, ACT_DIM) for g in train_graphs]
    test_list  = [graph_to_data(g, ACT_DIM) for g in test_graphs]
    # val_list  = [graph_to_data(g, ACT_DIM) for g in val_graphs]
    ...
    total, halluc_count, ratio = compute_hallucination_ratio(train_list)
    ...
    print("AUROC std:", np.std(auprc_results))
