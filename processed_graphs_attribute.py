...
def process_single_file(args):
    fname, input_dir, out_dir = args
    sample_path = os.path.join(input_dir, fname)
    if os.path.exists(os.path.join(out_dir, f"graph_{fname}")):
        return fname
    # ----- 逐条加载 -----
...
if __name__ == "__main__":
    ...
    input_dir = "attention_hidden_data/RAGtruth/Llama-3.1-8B-Instruct/train_QA"
    
    # ===== 输出目录 =====
    out_dir = "attributed_graphs_fixed/RAGtruth/Llama-3.1-8B-Instruct/train_QA"
    ...
    args = [(f, input_dir, out_dir) for f in files]
    NUM_WORKERS = min(8, os.cpu_count())  # 合理并发，不要过高以免 I/O 抢占

    with Pool(processes=NUM_WORKERS) as pool:
        for _ in tqdm(pool.imap_unordered(process_single_file, args), total=len(files)):
            pass
