...
def get_token_hallucination_labels(seq_len, hallucination_spans):
    ...
    for s, e in hallucination_spans:
        ...
        for i in range(s, e):
            labels[i] = 1
    return labels
# model_name="../../models/Mistral-7B-Instruct-v0.3" #Qwen2.5-7B-Instruct
model_name="../../models/llama2-7b-chat-hf"
...
print(f"Starting extract {split_type}_{task_type} ......")
save_dir = f"attention_hidden_data/Text-sql/Qwen2.5-7B-Instruct/{split_type}_{task_type}"
import os
os.makedirs(save_dir, exist_ok=True)

for i in tqdm(range(len(response)),desc="Data processing"):
    if response[i]['model'] == "qwen-2.5-7b" and response[i]["split"] == split_type:  ##QA test数据总共450条，其中有幻觉的为226条
        ...
        if source_info_dict[source_id]['task_type']==task_type:#train 839条QA
            ...
            prefix_seq = prefix_ids[0]
            
            prefix_len = prefix_seq.size(0)
            
            # # 对齐检查
            # is_same = torch.equal(input_seq[:prefix_len], prefix_seq)
            
            # print("Prefix match:", is_same)
            
            # if not is_same:
            #     print("Input prefix tokens:", input_seq[:prefix_len])
            #     print("Expected prefix tokens:", prefix_seq)
            # else:
            #     print("is Same")
            # break
